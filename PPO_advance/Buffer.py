...
class Buffer_for_PPO_2:
    ...
    def __init__(self, capacity, obs_dim, act_dim, device,trick = None):
        ...
        self.values = np.zeros(capacity)  # 用于存储状态值
        ...
        # 新增字段
        self.values = np.zeros(capacity)  # 存储状态值估计
        self.advantages = np.zeros(capacity)  # 存储优势函数值
        self.returns = np.zeros(capacity)  # 存储回报值

    def add(self, obs, action, reward, next_obs, done, action_log_probs, adv_done, value):
        """ add an experience to the memory """ #
        self.obs[self._index] = obs
        self.actions[self._index] = action
        self.rewards[self._index] = reward
    ...
    def compute_returns_and_advantage(self, gamma , gae_lambda ,last_value):
        """
        参考sb3的同名函数
        见：https://github.com/DLR-RM/stable-baselines3/blob/master/stable_baselines3/common/buffers.py#L403
        仍可改进的地方(对非episode任务) 见：https://github.com/DLR-RM/stable-baselines3/blob/master/stable_baselines3/common/on_policy_algorithm.py#L236

        计算每一步的回报和优势函数
        :param last_value: 最后一个状态的价值估计
        ...
        for step in reversed(range(self._size)):
            ...
            last_gae_lam = delta + gamma * gae_lambda * (1.0 - self.adv_dones[step]) * last_gae_lam
    ...
    def all(self):
        ...
        values = torch.as_tensor(self.values,dtype=torch.float32).reshape(-1,1).to(self.device)

        
        return obs, actions, rewards, next_obs, dones , actions_log_probs, adv_dones ,values